...
useEffect(() => {
    ...
    setUserId(storedId);
    // Retrieve posts from localStorage and ensure the status field is valid
    const savedPosts = JSON.parse(localStorage.getItem('posts') || '[]');
    const validatedPosts = savedPosts.map((post: any) => ({
    ...post,
      // Ensure that status is one of the valid values ('active', 'completed', 'pinned')
    status: post.status === 'active' || post.status === 'completed' || post.status === 'pinned' ? post.status : 'active',
    })) as PostData[];
